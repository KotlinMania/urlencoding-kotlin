# Claude Code Project Instructions — urlencoding-kotlin

## Project Overview

This is **urlencoding-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`urlencoding`](https://crates.io/crates/urlencoding).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/urlencoding/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
