...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Urlencoding.swiftmodule/`
...
//      `import Urlencoding` above would fail at compile time.
//
//   2. The static archive `libUrlencoding.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Urlencoding" }` and
//      `flattenPackage = "io.github.kotlinmania.urlencoding"` configuration in
...
//      via the `UrlencodingLibrary` product.
//
...
final class UrlencodingExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Urlencoding swift module imported cleanly")
    }
}
